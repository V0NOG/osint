...
export default function WorldError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => { console.error('[world error]', error) }, [error])
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
      ...
      <h2 className="text-lg font-bold text-[var(--color-text-primary)] mb-2">Globe unavailable</h2>
      <p className="text-sm text-[var(--color-text-secondary)] max-w-sm leading-relaxed mb-6">
        The world overview failed to load. This may be a WebGL or data issue.
      </p>
      <button
        onClick={reset}
